...
ACTUAL_VRAM_GB = 4  # Total VRAM per replica (model + KV cache + overhead)
...
def main():
    ...
    for gpu_key, info in state.items():
        ...
        if gpu_available >= ACTUAL_VRAM_GB:
            ...
            replicas_per_gpu.append((gpu_key, gpu_replicas, gpu_available))
    
    max_replicas = sum(replicas for _, replicas, _ in replicas_per_gpu)
    ...
    print(f"  Replicas per GPU (based on available VRAM):")
    for gpu_key, gpu_replicas, gpu_available in replicas_per_gpu:
        print(f"    {gpu_key}: {gpu_replicas} replicas ({gpu_available:.2f}GB available)")
